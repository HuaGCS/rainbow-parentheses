...
/**
 * JSON 键名按嵌套深度着色 pass。深度 = 该属性外层 [JsonObject] 的数量 − 1（顶层键为 0）。
 *
 * @author Hua
 */
class JsonRainbowKeysPass(file: PsiFile, editor: Editor) :
        ...
        file.accept(object : PsiRecursiveElementVisitor() {
            override fun visitElement(element: PsiElement) {
                if (element is JsonProperty) {
                    val depth = (ancestorDepth(element, JsonObject::class.java) - 1).coerceAtLeast(0)
                    highlightFor(element.nameElement, RainbowColorsManager.getKeyColorKey(depth))
                        ?.let { result.add(it) }
                }
    ...
    private companion object {
        val KEY_HIGHLIGHTERS_KEY =
            Key.create<MutableList<RangeHighlighter>>("RAINBOW_JSON_KEY_HIGHLIGHTERS")
    }
}
