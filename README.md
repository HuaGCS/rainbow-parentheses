...
A plugin for IntelliJ Platform IDEs that colorizes brackets, indent guides and
the current scope by nesting level.

Licensed under the Apache License 2.0. See [LICENSE](LICENSE) and [NOTICE](NOTICE).

<!-- Plugin description -->
Rainbow Parentheses colorizes brackets, indent guides, and the current scope by
nesting level for IntelliJ Platform IDEs. Free and open source.
...
<ul>
  ...
  <li><b>Rainbow lambda arrows</b>: colorize Kotlin lambda <code>-&gt;</code> arrows by lambda nesting depth. Off by default.</li>
...
<ul>
  <li>Settings: <code>Settings → Tools → Rainbow Parentheses</code> — master switch, per-bracket toggles, indent / scope toggles, color count, big-file threshold, exclusion lists.</li>
  <li><code>Ctrl + Shift + R</code>: master enable / disable.</li>
  <li><code>Ctrl + right mouse click</code>: highlight the current bracket scope.</li>
</ul>

<h3>Note</h3>
<p>Angle brackets <code>&lt;&gt;</code> are matched as plain text and may misfire on generics / comparison operators, so they are disabled by default and can be enabled in settings if you wish.</p>

<hr/>

<h3>中文说明</h3>

<p>为 IntelliJ Platform 系列 IDE 提供的彩虹括号 / 彩虹缩进线 / 作用域高亮插件，
按嵌套层级给括号着色、给原生缩进引导线上彩虹色，并支持高亮光标所在作用域。</p>
...
<ul>
  ...
  <li><b>彩虹 lambda 箭头</b>：Kotlin lambda 的 <code>-&gt;</code> 箭头按 lambda 嵌套深度着色。默认关闭。</li>
...
resources from any third-party plugin. Functional concepts that are common to
the "rainbow bracket" / "colored indent guide" idea generally are implemented
from scratch in this codebase (text-based bracket matching, custom
`TextEditorHighlightingPass` for bracket coloring, a custom action for scope
highlighting, etc.).

The only third-party code influence is the geometry of the indent-guide
renderer, which is derived from JetBrains IntelliJ Community Edition
(`com.intellij.codeInsight.daemon.impl.IndentGuideRenderer`, Apache License
2.0). The derivation, file location and license terms are documented in
[NOTICE](NOTICE) and in the file header of
`src/main/kotlin/com/hua/rainbow_parentheses/indents/RainbowIndentGuideRenderer.kt`.

All color values (in `src/main/resources/colorSchemes/`) and
`TextAttributesKey` external names (`HUA_RAINBOW_*`) are original to this
project and were not copied from any other plugin.

---

> The remainder of this README contains notes inherited from the IntelliJ
> Platform Plugin Template used to scaffold this project, kept for development
> reference. They do not describe end-user-facing features of this plugin.
